...
const MerchantCounters = ({counters, merchantName}) => {
  return (
    <div>
        <h1 className="text-4xl font-bold mb-6 text-center text-gray-800">Counters owned by {merchantName}</h1>
                ...
                
                {
                    counter?.merchant.length > 0 && (
                        counter?.merchant.map((merchant)=>(
                            <p className="text-lg text-gray-600 mb-2">{merchant.name}</p>
                        ))
                    )
                }
              </div>
